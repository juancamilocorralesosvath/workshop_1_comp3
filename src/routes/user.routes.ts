...
import { validate, updateUserSchema, assignRoleSchema } from '../middleware/validation.middleware';

export const userRouter = Router();
const userController = new UserController();

userRouter.get('/', authenticate, authorize(['admin', 'recepcionista']), userController.getAllUsers);
userRouter.get('/:id', authenticate, authorize(['admin', 'recepcionista', 'coach']), userController.getUserById);
userRouter.post('/', authenticate, authorize(['admin', 'recepcionista']), userController.createUser);
userRouter.put('/:id', authenticate, authorize(['admin']), validate(updateUserSchema), userController.updateUser);
